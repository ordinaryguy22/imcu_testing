...
module rv32i_soc #(
    ...
    wire Master_Route_Out;
    wire Master_Route_In;

    wire [31:0] master_addr;     // Master address
    wire [31:0] master_wdata;    // Master write data
    wire        master_write;    // Master write enable
    wire        master_read;     // Master write enable
    wire [2:0]  master_op;       // Master operation
    wire [31:0] master_rdata;    // Master read data
    logic dma_stall;
    // ============================================
    ...
    wire        wb_s2m_clint_rty;
        ...
        // IMAX
    wire [31:0] wb_m2s_imax_adr;
    wire [31:0] wb_m2s_imax_dat;
    wire  [3:0] wb_m2s_imax_sel;
    wire        wb_m2s_imax_we;
    wire        wb_m2s_imax_cyc;
    wire        wb_m2s_imax_stb;
    wire  [2:0] wb_m2s_imax_cti;
    wire  [1:0] wb_m2s_imax_bte;
    wire [31:0] wb_s2m_imax_dat;
    wire        wb_s2m_imax_ack;
    wire        wb_s2m_imax_err;
    wire        wb_s2m_imax_rty;
    // PTC
    ...
    logic dma_has_access;   
    assign master_addr  =  dma_has_access ? DMA_Address   : mem_addr_mem  ;
    assign master_wdata =  dma_has_access ? DMA_WriteData : mem_wdata_mem ;
    assign master_write =  dma_has_access ? DMA_Write     : mem_write_mem ;
    assign master_read  =  dma_has_access ? DMA_Read      : mem_read_mem  ;
    assign master_op    =  dma_has_access ? 3'b010        : mem_op_mem    ;
    assign DMA_ReadData   = dma_has_access ? master_rdata : 'bz;
    assign mem_rdata_mem  = dma_has_access ? 'bz : master_rdata;   
    
   
    // In Memory Accelerator Exchange 
    ...
    IMAX  InMemory_Accelerator_Exchange(                                  
             ...
             .clk_i(clk),         // clock
             .reset_n(reset_n),
             .cyc_i(wb_m2s_imax_cyc),         // cycle
             .stb_i(wb_m2s_imax_stb),         // strobe
             .adr_i(wb_m2s_imax_adr),         // address
             .we_i(wb_m2s_imax_we),          // write enable
             .dat_i(wb_m2s_imax_dat),         // data input
             .dat_o(wb_s2m_imax_dat),         // data output
             .ack_o(wb_s2m_imax_ack),          // normal bus termination
    ...
    assign external_irq = DMA_ireq;    
    
    
    logic wishbone_stall_pipl;


    assign stall_pipl = (dma_has_access) ? (mem_write_mem | mem_read_mem) : (wishbone_stall_pipl);
    assign dma_stall  = (~dma_has_access) ? (wishbone_stall_pipl)          : (1'b0);  
    ...
    wishbone_controller wishbone_master (
        ...
        .proc_addr  (master_addr),
        .proc_wdata (master_wdata),
        .proc_write (master_write),
        .proc_read  (master_read),
        .proc_op    (master_op),
        .proc_rdata (master_rdata),
        .proc_ack   (mem_ack_mem),
        .proc_stall_pipl(wishbone_stall_pipl), // Stall pipeline if needed
...
// PTC
    ...
    // IMAX
    .wb_imax_adr_o       (wb_m2s_imax_adr),
    .wb_imax_dat_o       (wb_m2s_imax_dat),
    .wb_imax_sel_o       (wb_m2s_imax_sel),
    .wb_imax_we_o        (wb_m2s_imax_we),
    .wb_imax_cyc_o       (wb_m2s_imax_cyc),
    .wb_imax_stb_o       (wb_m2s_imax_stb),
    .wb_imax_cti_o       (wb_m2s_imax_cti),
    .wb_imax_bte_o       (wb_m2s_imax_bte),
    .wb_imax_dat_i       (wb_s2m_imax_dat),
    .wb_imax_ack_i       (wb_s2m_imax_ack),
    .wb_imax_err_i       (wb_s2m_imax_err),
    .wb_imax_rty_i       (wb_s2m_imax_rty));  
    // ============================================
    //                  SPI Flash
    // ============================================   
